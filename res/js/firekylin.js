(function (win, doc) {
  ...
  /*
  win.addEventListener('load', setHTMLBg);
  win.addEventListener('resize', setHTMLBg);
  function setHTMLBg() {
    const pw = document.documentElement.clientWidth;
    const ph = document.documentElement.clientHeight;
    const compare = 1920 / 1080;
    console.log(pw / ph);
    if (pw / ph >= compare) {
      console.log("set width");
      document.getElementsByTagName("html")[0].style.setProperty("background-size", "100% auto");
    } else {
      console.log("set height");
      document.getElementsByTagName("html")[0].style.setProperty("background-size", "auto 100%");
    }
  }
  */
